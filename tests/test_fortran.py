...
class FortranTestWrapper:
    ...
    def clean(self):
        ...
        files_to_delete = glob.glob(
            os.path.join(os.path.split(__file__)[0], '{}*.f95').format(
                self.TEST_MODULE_NAME
            )
        ) + glob.glob(
            os.path.join(os.path.split(__file__)[0], '{}*.*.so').format(
                self.TEST_MODULE_NAME
            )
        )
    ...
    def build_model(symbols, test_module_name):
        ...
        output = subprocess.run(
            ['f2py', '-c', '{}.f95'.format(test_module_name), '-m', test_module_name],
...
class TestModelContainerMethods(
    FortranTestWrapper, test_core.TestModelContainerMethods
):
...
class TestSolutionErrorHandling(
    FortranTestWrapper, test_core.TestSolutionErrorHandling
):
...
class TestLinkerInit(FortranTestWrapper, test_core.TestLinkerInit):
    ...
    def setUp(self):
        self.SubmodelNoLags = self.build_model(
            self.SYMBOLS_NO_LAGS, 'fsic_test_fortran_testlinkerinit_no_lags'
        )
        self.SubmodelWithLags = self.build_model(
            self.SYMBOLS_WITH_LAGS, 'fsic_test_fortran_testlinkerinit_with_lags'
        )
        self.SubmodelWithLeads = self.build_model(
            self.SYMBOLS_WITH_LEADS, 'fsic_test_fortran_testlinkerinit_with_leads'
        )
...
class TestBuild(unittest.TestCase):
    def test_lags(self):
        ...
        self.assertIn(
            ...
            fsic.fortran.build_fortran_definition([], lags=2),
        )
    ...
    def test_leads(self):
        ...
        self.assertIn(
            ...
            fsic.fortran.build_fortran_definition([], leads=2),
            2,
        )
    ...
    def test_min_lags(self):
        ...
        self.assertIn(
            'integer :: lags = 2, leads = 0',
            fsic.fortran.build_fortran_definition([], min_lags=2),
        )
    ...
    def test_min_leads(self):
        ...
        self.assertIn(
            'integer :: lags = 0, leads = 2',
            fsic.fortran.build_fortran_definition([], min_leads=2),
        )
...
"""
    ...
    def test_evaluate(self):
        ...
        # Comparison
        self.assertEqual(
            self.model_python.values.shape, self.model_fortran.values.shape
        )
        self.assertTrue(
            np.allclose(self.model_python.values, self.model_fortran.values)
        )
