...
class TestFunctions(unittest.TestCase):
    def test_lag(self):
        ...
        self.assertTrue(
            np.allclose(x, np.array([0.0, 1.0, 2.0, 3.0, 4.0], dtype=float))
        )
...
class TestFunctionsEval(unittest.TestCase):
    def test_eval_lag(self):
        ...
        self.assertTrue(
            np.allclose(container.x, np.array([0.0, 1.0, 2.0, 3.0, 4.0], dtype=float))
        )

        self.assertTrue(
            np.array_equal(
                container.eval('lag(x)'),
                np.array([np.nan, 0.0, 1.0, 2.0, 3.0], dtype=float),
                equal_nan=True,
            )
        )
