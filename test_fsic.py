...
class TestOrder(unittest.TestCase):
    ...
    def test_order_sorted(self):
        # Test that `build_model_definition()` correctly orders the equation
        # statements to be in alphabetical order using `sorted()`
        model = '''
C = {alpha_1} * YD + {alpha_2} * H[-1]
YD = Y - T
Y = C + G
...
'''
        ...
        script = fsic.build_model_definition(symbols, order=sorted)
    ...
    def test_order_sorted_reverse(self):
        # Test that `build_model()` correctly orders the equation statements to
        # be in reverse alphabetical order using `sorted()` (with
        # `reverse=True`)
        model = '''
C = {alpha_1} * YD + {alpha_2} * H[-1]
YD = Y - T
...
'''
        ...
        SIM = fsic.build_model(symbols,
                               order=functools.partial(sorted, reverse=True))
